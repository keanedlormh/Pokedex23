/**
 * Datos del Modelo: S95TR
 *
 * Define el objeto para un único producto y lo registra
 * en la base de datos global.
 */

const S95TR = {
    // Clave del modelo
    "model": "S95TR",
    
    // Clave del esquema que debe usar (debe coincidir con la clave de registro)
    "schema_key": "soundbars",

    // Objeto de atributos (solo los que tiene, el resto se omiten)
    "attributes": {
        "general_num_canales": "9.1.5",
        "general_potencia_salida_w": "810",
        "general_num_altavoces": "17",
        "dim_principal_mm": "1250 x 63 x 135",
        "dim_subwoofer_mm": "201 x 407 x 403",
        "dim_traseros_mm": "159 x 221 x 142",
        "con_hdmi_in": "2",
        "con_hdmi_out": "1",
        "con_usb": "1",
        "con_optica": "1",
        "con_bluetooth_version": "5.1",
        "con_wifi": "Si",
        "con_chromecast": "Si",
        "con_airplay2": "Si",
        "con_spotify_connect": "Si",
        "con_tidal_connect": "Si",
        "con_google_home": "Si",
        "con_alexa": "Si",
        "hdmi_earc": "Si",
        "hdmi_cec_simplink": "Si",
        "hdmi_passthrough_4k": "Si",
        "hdmi_vrr_allm": "Si",
        "hdmi_120hz": "Si",
        "hdmi_hdr10": "Si",
        "hdmi_dolby_vision": "Si",
        "hdmi_version": "2.1",
        "audio_dolby_atmos": "Si",
        "audio_dolby_digital": "Si",
        "audio_dtsx": "Si",
        "audio_aac": "Si",
        "audio_mqa": "Si",
        "hires_muestreo": "24bit/192kHz",
        "hires_upsampling": "Si",
        "modo_ai_sound_pro": "Si",
        "modo_standard": "Si",
        "modo_cinema": "Si",
        "modo_game": "Si",
        "modo_musica": "Si",
        "modo_clear_voice_pro": "Si",
        "modo_deportes": "Si",
        "modo_bass_blast": "Si",
        "smart_remote_app": "Si",
        "smart_ai_room_calibration": "Si",
        "smart_control_modos_barra": "Si",
        "smart_tv_sound_mode_share": "Si",
        "smart_wow_interface": "Si",
        "smart_wow_orchestra": "Si",
        "acc_mando_distancia": "Si",
        "acc_tarjeta_garantia": "Si",
        "acc_cable_hdmi": "Si",
        "acc_cable_optico": "Si",
        "acc_soporte_pared": "Si",
        "acc_pilas": "Si"
    }
};

// --- REGISTRO ---
// Comprueba si la BD global existe y registra este producto
if (window.APP_DB && typeof window.APP_DB.registerProduct === 'function') {
    window.APP_DB.registerProduct(S95TR);
} else {
    console.error("Error: APP_DB no está inicializada. Asegúrate de que main.js se carga primero.");
}