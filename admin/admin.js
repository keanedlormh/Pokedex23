/*
 * Lógica del Panel de Administración v3.4.4
 * Update: Importación Texto en Biblioteca
 */
...
document.addEventListener('DOMContentLoaded', () => {
    ...
    const dom = {
        ...
        // NUEVO: Referencias de importación de texto
    ...
    // ... (Funciones de Tema y CSS, omitidas para brevedad - IGUALES AL ANTERIOR) ...
    function applyInitialTheme() { const savedMode = localStorage.getItem('admin-theme-mode'); isLightMode = (savedMode === 'light'); currentAccentHue = Math.floor(Math.random() * 360); if (isLightMode) updatePaletteCSS(lightPaletteHSL, currentAccentHue); else updatePaletteCSS(darkPaletteHSL, currentAccentHue); }
    function handleThemeToggle(e) { if (e) e.stopPropagation(); isLightMode = !isLightMode; localStorage.setItem('admin-theme-mode', isLightMode ? 'light' : 'dark'); currentAccentHue = Math.floor(Math.random() * 360); if (isLightMode) updatePaletteCSS(lightPaletteHSL, currentAccentHue); else updatePaletteCSS(darkPaletteHSL, currentAccentHue); }
    function updatePaletteCSS(baseHSL, accentHue) { const p = baseHSL; const newOtherHue = (accentHue + 0 + 360) % 360; const accentRGB = hslToRgb(accentHue, p.accent.s, p.accent.l); const root = document.documentElement; const vars = { '--color-cyan-accent': `hsl(${accentHue}, ${p.accent.s}%, ${p.accent.l}%)`, '--color-cyan-glow': `rgba(${accentRGB.r}, ${accentRGB.g}, ${accentRGB.b}, 0.25)`, '--color-green-accent': `hsl(${ (accentHue + 120) % 360 }, 80%, 45%)`, '--color-red-accent': `hsl(${ (accentHue + 240) % 360 }, 85%, 55%)`, '--color-bg-dark': `hsl(${newOtherHue}, ${p.dark.s}%, ${p.dark.l}%)`, '--color-bg-medium': `hsl(${newOtherHue}, ${p.medium.s}%, ${p.medium.l}%)`, '--color-border': `hsl(${newOtherHue}, ${p.border.s}%, ${p.border.l}%)`, '--color-border-light': `hsl(${newOtherHue}, ${p.border.s}%, ${p.border.l + 5}%)`, '--color-text-primary': `hsl(${newOtherHue}, ${p.textP.s}%, ${p.textP.l}%)`, '--color-text-secondary': `hsl(${newOtherHue}, ${p.textS.s}%, ${p.textS.l}%)`, '--color-text-dim': `hsl(${newOtherHue}, ${p.textS.s}%, ${p.textS.l - 10}%)` }; for (const [key, value] of Object.entries(vars)) root.style.setProperty(key, value); if (isLightMode) root.classList.add('light-mode'); else root.classList.remove('light-mode'); }
    ...
    function handleSaveClick() { if (currentActivePanel === 'edit-model') exportDataFromEditor(); else if (currentActivePanel === 'edit-schema') handleSchemaExportClick(); }
    function showPanel(panelId) { currentActivePanel = panelId; dom.allContentPanels.forEach(p => p.classList.remove('active')); document.body.classList.remove('fullscreen-editor-active'); dom.saveBtn.style.display = 'none'; if (dom.settingsMenu) dom.settingsMenu.style.display = 'none'; const target = document.getElementById(`panel-${panelId}`); if (target) target.classList.add('active'); if (panelId === 'edit-model' || panelId === 'edit-schema') { document.body.classList.add('fullscreen-editor-active'); dom.saveBtn.style.display = 'inline-flex'; } }
    ...
    function handleGamaToggle(e) { if (!e.target.matches('.gama-checkbox')) return; const key = e.target.dataset.key; if (e.target.checked) activeSchemas.add(key); else activeSchemas.delete(key); const statusSpan = e.target.closest('.gama-toggle-item').querySelector('.gama-status'); statusSpan.textContent = e.target.checked ? 'Activa' : 'Oculta'; statusSpan.style.color = e.target.checked ? 'var(--color-cyan-accent)' : 'var(--color-text-dim)'; refreshUI(); }
    function handleCsvUpload(e) { const file = e.target.files[0]; if (!file) return; dom.uploadStatusText.textContent = `Leyendo: ${file.name}...`; const reader = new FileReader(); reader.onload = (event) => { try { parseAndImportCsv(event.target.result); dom.uploadStatusText.textContent = `¡Importado!`; dom.uploadStatusText.style.color = 'var(--color-cyan-accent)'; refreshUI(); } catch (error) { console.error(error); dom.uploadStatusText.textContent = `Error: Formato inválido`; dom.uploadStatusText.style.color = '#ef4444'; alert("Error CSV: Asegúrate de usar el formato 3 filas."); } }; reader.readAsText(file, 'UTF-8'); }
    
    // --- NUEVO: Importación Texto ---
    function handleTextImport() {
        const text = dom.csvTextInput.value.trim();
        if (!text) return;
        dom.uploadStatusText.textContent = "Procesando...";
        try {
    ...
    // ... (Resto de lógica igual, filtrado search, editores) ...
    function applySearch() { const q = dom.modelSearchInput.value.toLowerCase().trim(); const filtered = masterDatabase.filter(p => p.model.toLowerCase().includes(q) && activeSchemas.has(p.schema_key)); renderSearchResults(filtered); }
    function renderSearchResults(results) { dom.modelSearchResults.innerHTML = ''; if (results.length === 0) { dom.modelSearchResults.innerHTML = '<div class="list-item dim">No se encontraron resultados (o gama oculta).</div>'; return; } const frag = document.createDocumentFragment(); results.forEach(p => { const item = document.createElement('div'); item.className = 'list-item'; item.dataset.model = p.model; item.innerHTML = `<strong>${p.model}</strong> <span class="dim text-xs">(${p.schema_key})</span>`; frag.appendChild(item); }); dom.modelSearchResults.appendChild(frag); }
    ...
    function populateSchemaList() { dom.schemaResultsList.innerHTML = ''; const frag = document.createDocumentFragment(); Object.keys(masterSchemaMap).filter(k => activeSchemas.has(k)).forEach(key => { const item = document.createElement('div'); item.className = 'list-item'; item.dataset.schemaKey = key; item.innerHTML = `<strong>${key}</strong>`; frag.appendChild(item); }); dom.schemaResultsList.appendChild(frag); }
    ...
    function handleCreateModelClick() { const schema = dom.createModelSchemaSelect.value; const id = dom.createModelIdInput.value.trim().toUpperCase(); if (!schema) return alert("Selecciona una gama."); if (!id) return alert("Introduce un Model ID."); if (/\s/.test(id)) return alert("El ID no puede tener espacios."); const existing = masterDatabase.find(p => p.model === id); if (existing) { if(!confirm(`El modelo ${id} ya existe. ¿Editarlo?`)) return; loadModelIntoEditor(existing); showPanel('edit-model'); return; } loadModelIntoEditor({ model: id, schema_key: schema, attributes: {} }); dom.createModelIdInput.value = ''; showPanel('edit-model'); }
    ...
    function populateGamaSelectors() { const frag = document.createDocumentFragment(); Object.keys(masterSchemaMap).filter(k => activeSchemas.has(k)).forEach(key => { const opt = document.createElement('option'); opt.value = key; opt.textContent = key.charAt(0).toUpperCase() + key.slice(1); frag.appendChild(opt); }); if (dom.gamaExportSelect) { dom.gamaExportSelect.innerHTML = '<option value="">-- Seleccionar --</option>'; dom.gamaExportSelect.appendChild(frag.cloneNode(true)); } if (dom.createModelSchemaSelect) { dom.createModelSchemaSelect.innerHTML = '<option value="">-- Seleccionar Gama --</option>'; dom.createModelSchemaSelect.appendChild(frag.cloneNode(true)); } }
    function populateGamaExportList() { const schema = dom.gamaExportSelect.value; dom.gamaExportList.innerHTML = ''; if (!schema) { dom.exportGamaJsonButton.disabled = true; dom.exportGamaCsvButton.disabled = true; return; } dom.exportGamaJsonButton.disabled = false; dom.exportGamaCsvButton.disabled = false; const prods = masterDatabase.filter(p => p.schema_key === schema); if (prods.length === 0) { dom.gamaExportList.innerHTML = '<p class="dim p-2">Sin modelos.</p>'; return; } const frag = document.createDocumentFragment(); prods.forEach(p => { const div = document.createElement('div'); div.className = 'gama-export-item'; div.innerHTML = `<span>${p.model}</span><button class="export-item-button" data-model="${p.model}">JSON</button>`; frag.appendChild(div); }); dom.gamaExportList.appendChild(frag); }
    ...
    function exportGamaAsCsv() { const selectedSchema = dom.gamaExportSelect.value; if (!selectedSchema) return; const products = masterDatabase.filter(p => p.schema_key === selectedSchema); if (products.length === 0) return alert("Gama vacía."); const schemaDef = masterSchemaMap[selectedSchema]; let row1_groups = [selectedSchema, "Identificación"]; let row2_codes = ["", "model"]; let row3_descs = ["", "Modelo"]; let attrKeys = []; if (schemaDef) { schemaDef.forEach(group => { group.attrs.forEach(attr => { row1_groups.push(group.group); row2_codes.push(attr.code); row3_descs.push(attr.desc); attrKeys.push(attr.code); }); }); } else { const allKeys = new Set(); products.forEach(p => Object.keys(p.attributes).forEach(k => allKeys.add(k))); Array.from(allKeys).forEach(key => { row1_groups.push("Datos"); row2_codes.push(key); row3_descs.push(key); attrKeys.push(key); }); } const sanitize = (val) => { if (val === null || val === undefined) return ""; val = String(val); val = val.replace(/"/g, '""'); if (val.search(/("|\;|:|\n|\r)/g) >= 0) val = `"${val}"`; return val; }; const dataRows = products.map(p => { let row = ["", sanitize(p.model)]; attrKeys.forEach(key => { let val = p.attributes[key] || ""; row.push(sanitize(val)); }); return row.join(";"); }); const header1 = row1_groups.map(g => sanitize(g)).join(";"); const header2 = row2_codes.map(c => sanitize(c)).join(";"); const header3 = row3_descs.map(d => sanitize(d)).join(";"); const csvContent = "\uFEFF" + header1 + "\n" + header2 + "\n" + header3 + "\n" + dataRows.join("\n"); const filename = `GAMA_${selectedSchema.toUpperCase()}.csv`; downloadFile(filename, csvContent, 'text/csv;charset=utf-8'); }
    function loadModelIntoEditor(product) { currentLoadedSchemaKey = product.schema_key; const schema = masterSchemaMap[product.schema_key]; if (!schema) return alert(`Esquema ${product.schema_key} no encontrado.`); const isNew = Object.keys(product.attributes).length === 0; dom.productTitle.textContent = isNew ? `Crear: ${product.model}` : `Editar: ${product.model}`; dom.editModelIdInput.value = product.model; dom.editSchemaKeyDisplay.value = product.schema_key; dom.editorForm.querySelectorAll('.form-group-title, .form-row').forEach(el => el.remove()); dom.editorPlaceholder.style.display = 'none'; const frag = document.createDocumentFragment(); schema.forEach(group => { const h3 = document.createElement('h3'); h3.className = 'form-group-title'; h3.textContent = group.group; frag.appendChild(h3); group.attrs.forEach(attr => { const val = product.attributes[attr.code] || ""; const row = document.createElement('div'); row.className = 'form-row'; const uniqueId = `attr_${attr.code}_${Math.random().toString(36).substr(2,5)}`; const label = document.createElement('label'); label.className = 'futuristic-label'; label.htmlFor = uniqueId; label.textContent = `${attr.desc} (${attr.code})`; const ta = document.createElement('textarea'); ta.className = 'futuristic-textarea'; ta.id = uniqueId; ta.name = attr.code; ta.rows = 1; ta.value = val; ta.addEventListener('input', () => { ta.style.height = 'auto'; ta.style.height = (ta.scrollHeight) + 'px'; }); row.appendChild(label); row.appendChild(ta); frag.appendChild(row); }); }); dom.editorForm.appendChild(frag); setTimeout(() => { dom.editorForm.querySelectorAll('textarea').forEach(t => { t.style.height = 'auto'; t.style.height = (t.scrollHeight) + 'px'; }); }, 10); }
    function exportDataFromEditor() { if (!currentLoadedSchemaKey) return; const id = dom.editModelIdInput.value.trim().toUpperCase(); if (!id) return alert("Falta Model ID."); const formData = new FormData(dom.editorForm); const attrs = {}; for (const [k, v] of formData.entries()) if (v.trim()) attrs[k] = v.trim(); generateAndDownloadProductFile({ model: id, schema_key: currentLoadedSchemaKey, attributes: attrs }, id); alert("JSON Generado."); }
    ...
    function addGroupToEditor(group = null) { const div = document.createElement('div'); div.className = 'schema-group-box'; const gName = group ? group.group : ''; div.innerHTML = `<div class="schema-group-header"><div class="control-group flex-grow"><label class="futuristic-label">Grupo:</label><input type="text" class="futuristic-input w-full" data-type="group-name" value="${gName}" placeholder="NOMBRE GRUPO"></div><button class="schema-action-btn schema-add-attr-btn">+ Attr</button><button class="schema-action-btn schema-remove-btn schema-remove-group-btn">Eliminar</button></div><div class="schema-attributes-container"></div>`; const container = div.querySelector('.schema-attributes-container'); if (group && group.attrs) group.attrs.forEach(a => addAttributeToGroup(a, container)); dom.schemaEditorForm.appendChild(div); }
    function addAttributeToGroup(attr, container) { const div = document.createElement('div'); div.className = 'schema-attr-row'; const c = attr ? attr.code : ''; const d = attr ? attr.desc : ''; div.innerHTML = `<input type="text" class="futuristic-input" data-type="attr-code" value="${c}" placeholder="codigo"><input type="text" class="futuristic-input" data-type="attr-desc" value="${d}" placeholder="Descripción"><button class="schema-action-btn schema-remove-btn schema-remove-attr-btn">✕</button>`; container.appendChild(div); }
