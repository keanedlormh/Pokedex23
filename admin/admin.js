 ...
 * Update: Biblioteca Virtual en Admin + Fix AutoOpen
 */
...
document.addEventListener('DOMContentLoaded', () => {
    ...
    function refreshUI() {
        // Filtrar datos globales según activeSchemas
        renderSearchResults(masterDatabase);
        populateSchemaList();
        populateGamaSelectors();
        if (dom.libraryModal.style.display === 'block') {
    ...
    function renderLibraryGamaList() {
        ...
        Object.keys(masterSchemaMap).forEach(schemaKey => {
            ...
            item.className = 'gama-export-item'; // Reusar estilo lista
            item.innerHTML = `
                <label style="display:flex; align-items:center; gap:0.5rem; cursor:pointer; flex-grow:1;">
                    <input type="checkbox" class="gama-checkbox" data-key="${schemaKey}" ${isActive ? 'checked' : ''} style="accent-color:var(--color-cyan-accent);">
                    <span style="font-weight:600; color:var(--color-text-primary);">${friendlyName}</span>
                </label>
                <span class="status-text" style="font-size:0.7rem; font-weight:bold; color:${isActive ? 'var(--color-cyan-accent)' : 'var(--color-text-dim)'};">${isActive ? 'ACTIVA' : 'OCULTA'}</span>
            `;
    ...
    function handleGamaToggle(e) {
        ...
        if (e.target.checked) activeSchemas.add(key);
        else activeSchemas.delete(key);

        const statusSpan = e.target.closest('.gama-export-item').querySelector('.status-text');
    ...
    // --- CSV Import (Igual que Pokedex) ---
    function handleCsvUpload(e) {
        const file = e.target.files[0];
        if (!file) return;
        dom.uploadStatusText.textContent = `Leyendo: ${file.name}...`;
        const reader = new FileReader();
        reader.onload = (event) => {
            try {
                parseAndImportCsv(event.target.result);
                dom.uploadStatusText.textContent = `¡Importado!`;
                ...
                alert("Error CSV: Asegúrate de usar el formato 3 filas (Grupos/Códigos/Descripciones).");
            }
        };
    ...
    function parseAndImportCsv(csvText) {
        ...
        const newSchemaGroup = groupOrder.map(gName => ({
            group: gName, attrs: tempSchema[gName]
        }));
    ...
    function renderSearchResults(results) {
        ...
        if (results.length === 0) {
            dom.modelSearchResults.innerHTML = '<div class="list-item dim">No se encontraron resultados (o gama oculta).</div>';
            return;
        }
    ...
    function handleSchemaCreateClick() {
        ...
        if (masterSchemaMap[key]) { if (!confirm("Ya existe. ¿Cargar?")) return; loadSchemaIntoEditor(key, JSON.parse(JSON.stringify(masterSchemaMap[key]))); } else { loadSchemaIntoEditor(key, []); }
    ...
    function populateGamaExportList() {
        ...
        prods.forEach(p => {
            ...
            div.innerHTML = `<span>${p.model}</span><button class="export-item-button" data-model="${p.model}">JSON</button>`; frag.appendChild(div);
        });
