 ...
 * Update: Parser CSV 3-Filas (Grupos, Códigos, Descripciones).
 */

document.addEventListener('DOMContentLoaded', () => {

    // ... (Referencias DOM igual que v4.1) ...
    const dom = {
        body: document.body,
    ...
    // ... (Resto de funciones de filtros y UI iguales a v4.1) ...
    ...
    function populateSchemaSelector() {
        ...
        if (activeSchemas.has(currentValue)) { dom.schemaFilterSelect.value = currentValue; } else { dom.schemaFilterSelect.value = 'all'; }
    }
    ...
    function removeSchemaFilter() {
        if (dom.schemaFilterSelect) { dom.schemaFilterSelect.value = 'all'; populateSmartFilters('all'); applyFiltersAndSearch(); }
    }
    ...
    function toggleFilterGroup(titleElement) {
        const btn = titleElement.querySelector('.filter-toggle-btn'); const rowsContainer = titleElement.nextElementSibling; 
        if (rowsContainer && btn) {
            if (rowsContainer.classList.contains('collapsed')) { rowsContainer.classList.remove('collapsed'); rowsContainer.classList.add('expanded'); btn.classList.remove('gray'); btn.classList.add('blue');
            } else { rowsContainer.classList.remove('expanded'); rowsContainer.classList.add('collapsed'); btn.classList.remove('blue'); btn.classList.add('gray'); }
        }
    }
    ...
    function checkOverlay() {
        if (dom.smartFilterPanel.className === 'smart-filter-content-hidden' && dom.settingsMenuPanel.className === 'settings-menu-panel-hidden' && dom.readmeModal.className === 'modal-hidden' && dom.libraryModal.className === 'modal-hidden') { dom.filterOverlay.className = 'overlay-hidden'; }
    }

    // --- Biblioteca ---
    function openLibraryModal() { closeSettingsMenu(); closeFilterPanel(); closeReadmeModal(); renderLibraryGamaList(); dom.libraryModal.className = 'modal-visible'; dom.filterOverlay.className = 'overlay-visible'; dom.uploadStatusText.textContent = "Formato Admin Requerido (Punto y coma)"; dom.csvUploadInput.value = ""; }
    ...
    function handleGamaToggle(e) {
        ...
        statusSpan.textContent = e.target.checked ? 'Activa' : 'Oculta';
        statusSpan.style.color = e.target.checked ? 'var(--color-cyan-accent)' : 'var(--color-text-dim)';
    ...
    function handleCsvUpload(e) {
        ...
        reader.onload = (event) => {
            try {
                ...
                dom.uploadStatusText.textContent = `¡Importado con éxito!`; dom.uploadStatusText.style.color = 'var(--color-cyan-accent)';
                buildAttributeCache(); populateSchemaSelector(); applyFiltersAndSearch(); renderLibraryGamaList();
            } catch (error) { console.error(error); dom.uploadStatusText.textContent = `Error: Formato inválido`; dom.uploadStatusText.style.color = '#ef4444'; alert("Error al leer el CSV. Asegúrate de usar el formato de exportación del Admin (Punto y coma)."); }
        };
    ...
    function parseCSVLine(text) {
        ...
        for (let i = 0; i < text.length; i++) {
            ...
            if (inQuotes) {
                ...
                else if (char === '"') { inQuotes = false; } else { current += char; }
            } else {
                if (char === ';') { result.push(current); current = ''; }
                else if (char === '"') { inQuotes = true; } else { current += char; }
            }
        }
    ...
    function parseAndImportCsv(csvText) {
        ...
        const groupOrder = []; // Para mantener orden de inserción de grupos
        ...
        for (let i = 3; i < lines.length; i++) {
            ...
            const newProduct = { model: modelId, schema_key: schemaKey, attributes: attributes };
